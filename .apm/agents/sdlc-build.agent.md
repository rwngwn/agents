...
Otherwise skip and proceed to Step 4. When applicable, ask via the host's interactive question tool (options A/B).
...
- Present options via the host's interactive question tool: A) merge to main, B) push + create PR, C) keep branch, D) discard
